...
type PreviewMeta={hash:string;originalName:string;size:number;createdAt:number;summary:Summary};
type Preview=PreviewMeta&{buffer:Buffer};
type Manifest={hash?:string};

export class ImportService{
 private previews=new Map<string,Preview>();
 private summaryFor(parsed:Awaited<ReturnType<typeof parseSalesWorkbook>>):Summary{const dates=parsed.records.map(record=>record.date.getTime());return{sheetName:parsed.sheetName??'Hoja 1',headers:parsed.headers,records:parsed.records.length+parsed.errors.length,validRecords:parsed.records.length,errors:parsed.errors.slice(0,20),errorCount:parsed.errors.length,duplicates:parsed.duplicates,dateFrom:dates.length?new Date(Math.min(...dates)).toISOString().slice(0,10):null,dateTo:dates.length?new Date(Math.max(...dates)).toISOString().slice(0,10):null,sample:parsed.records.slice(0,5).map(record=>({fecha:record.date.toISOString().slice(0,10),pv:record.pointOfSale,numero:record.number,cliente:record.customer,producto:record.product,total:record.total,saldo:record.balance,estado:record.status,usuario:record.user}))}}
 async preview(buffer:Buffer,originalName:string,size:number){
  await this.cleanup();const parsed=await parseSalesWorkbook(buffer);if(!parsed.records.length)throw new Error('No se encontraron registros válidos');
  const token=crypto.randomUUID(),hash=hashBuffer(buffer),manifest=await persistentStorage.readJson<Manifest>('current.json').catch(()=>null),alreadyImported=manifest?.hash===hash,summary=this.summaryFor(parsed),meta={hash,originalName,size,createdAt:Date.now(),summary};
  if(persistentStorage.isCloud())await Promise.all([persistentStorage.write(`previews/${token}.xlsx`,buffer,'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'),persistentStorage.writeJson(`previews/${token}.json`,meta)]);else this.previews.set(token,{buffer,...meta});
 ...
 async confirm(token:string,username:string){
  ...
  await persistentStorage.write('current.xlsx',preview.buffer,'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  await persistentStorage.writeJson('current.json',{hash:preview.hash,originalName:preview.originalName,size:preview.size,records:preview.summary.validRecords,importedAt:new Date().toISOString(),username});
 ...
 private async cleanup(){
  ...
  if(!persistentStorage.isCloud())return;
  const files=await persistentStorage.list('previews/'),jsonFiles=files.filter(file=>file.key.endsWith('.json'));
  await Promise.all(jsonFiles.map(async file=>{const meta=await persistentStorage.readJson<PreviewMeta>(file.key).catch(()=>null);if(meta&&meta.createdAt>=limit)return;const base=file.key.slice(0,-5);await persistentStorage.remove([file.key,`${base}.xlsx`])}));
 }
}
