...
import { AlertTriangle,ArrowRight,BrainCircuit,CheckCircle2,Lightbulb,Link2,X } from 'lucide-react';
...
const savedConversation=()=>{try{const value=JSON.parse(sessionStorage.getItem(conversationKey)??'[]');return Array.isArray(value)?value as AIConversationTurn[]:[]}catch{return[]}};
const quick=[
...
export function RojasAIPage(){
 ...
 useEffect(()=>{try{if(turns.length)sessionStorage.setItem(conversationKey,JSON.stringify(turns));else sessionStorage.removeItem(conversationKey)}catch{/* La conversación activa continúa aunque el navegador alcance su cuota local. */}},[turns]);
 useEffect(()=>{threadRef.current?.scrollTo({top:threadRef.current.scrollHeight,behavior:'smooth'})},[turns]);
 ...
 return <main className="page ai-page">
  ...
  <section className="decision-grid"><article className="panel"><div className="panel-head"><h2><AlertTriangle/> Riesgos que requieren revisión</h2></div><div className="decision-list">{brief.risks.length?brief.risks.map(item=><div key={item.title}><strong>{item.title}</strong><p>{item.evidence}</p><small>Acción sugerida: {item.action}</small></div>):<p>No se detectaron riesgos relevantes con las reglas actuales.</p>}</div></article><article className="panel"><div className="panel-head"><h2><Lightbulb/> Oportunidades observadas</h2></div><div className="decision-list">{brief.opportunities.map(item=><div key={item.title}><strong>{item.title}</strong><p>{item.evidence}</p><small>Acción sugerida: {item.action}</small></div>)}</div></article></section>
   <section className="ai-workspace"><div className="ai-prompt"><BrainCircuit/><div><h2>Conversación con los datos</h2><p>Podés continuar cada respuesta con nuevas preguntas.</p></div>{turns.length>0&&<button className="secondary-button finish-conversation" onClick={finishConversation}><X/> FINALIZAR CONVERSACIÓN</button>}</div>{turns.length>0&&<div className="conversation-thread" ref={threadRef}>{turns.map((turn,index)=><article key={index}><div className="conversation-user"><strong>Vos</strong><p>{turn.question}</p></div><div className="conversation-ai"><strong>Rojas IA</strong><p>{turn.answer.answer}</p>{index===turns.length-1&&<details className="conversation-analysis"><summary>Ver hallazgos, acciones y evidencia</summary><div className="answer-columns"><section><h3>Hallazgos</h3>{turn.answer.findings.map(item=><div className="finding linked-card" key={item.id}><b className={item.importance}>{item.importance}</b><strong>{item.title}</strong><p>{item.detail}</p><button className="context-link" onClick={()=>openFinding(item.id)}><Link2/> Ver acciones y evidencia</button></div>)}</section><section><h3>Acciones priorizadas</h3>{[...turn.answer.actions].sort((a,b)=>a.priority-b.priority).map(item=><div className="action linked-card" key={item.id}><span>{item.priority}</span><div><strong>{item.action}</strong><p>{item.reason}</p></div></div>)}</section></div><section className="conversation-evidence"><h3>Evidencia utilizada</h3>{turn.answer.evidence.map(item=><button className="evidence-row" key={item.label} onClick={()=>{setSelectedActionId(null);setContextFindingIds(item.relatedFindingIds)}}><span><strong>{item.label}:</strong> {item.value} <small>({item.source})</small></span><ArrowRight/></button>)}</section>{turn.answer.limitations.length>0&&<section className="conversation-limitations"><h3>Limitaciones</h3>{turn.answer.limitations.map(item=><p key={item}>{item}</p>)}</section>}</details>}</div></article>)}</div>}<div className="quick-prompts">{quick.map(item=><button key={item} onClick={()=>ask(undefined,item)}>{item}</button>)}</div><form onSubmit={event=>ask(event)}><textarea value={question} onChange={event=>setQuestion(event.target.value)} placeholder={turns.length?'Continuá la conversación…':'Ejemplo: ¿qué clientes explican la caída y qué debería revisar primero?'} maxLength={600}/><button className="primary-button" disabled={busy||question.trim().length<3}>{busy?'ANALIZANDO…':turns.length?'CONTINUAR':'ANALIZAR'}<ArrowRight/></button></form></section>
