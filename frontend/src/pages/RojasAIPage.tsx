...
import { rojasAiApi,type AIAnswer,type DecisionBriefing } from '../services/rojasAiApi';
...
export function RojasAIPage(){
 ...
 const[brief,setBrief]=useState<DecisionBriefing|null>(null),[status,setStatus]=useState<any>(null),[answer,setAnswer]=useState<AIAnswer|null>(null),[question,setQuestion]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState(''),[contextFindingIds,setContextFindingIds]=useState<string[]>([]),[selectedActionId,setSelectedActionId]=useState<string|null>(null);
 ...
 useEffect(()=>{setBrief(null);setAnswer(null);setContextFindingIds([]);load()},[queryString]);
 async function ask(event?:FormEvent,preset?:string){event?.preventDefault();const value=preset??question;if(!value.trim())return;setQuestion(value);setBusy(true);setError('');setContextFindingIds([]);try{setAnswer(await rojasAiApi.ask(value,'general',queryString))}catch(e){setError(e instanceof Error?e.message:'No se pudo analizar')}finally{setBusy(false)}}
 ...
 return <main className="page ai-page">
  ...
  <section className="ai-workspace"><div className="ai-prompt"><BrainCircuit/><div><h2>Consultá los datos</h2><p>La respuesta incluye evidencia y limitaciones.</p></div></div><div className="quick-prompts">{quick.map(item=><button key={item} onClick={()=>ask(undefined,item)}>{item}</button>)}</div><form onSubmit={event=>ask(event)}><textarea value={question} onChange={event=>setQuestion(event.target.value)} placeholder="Ejemplo: ¿qué clientes explican la caída y qué debería revisar primero?" maxLength={600}/><button className="primary-button" disabled={busy||question.trim().length<3}>{busy?'ANALIZANDO…':'ANALIZAR'}<ArrowRight/></button></form></section>
